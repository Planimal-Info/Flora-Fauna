...
router.post("/filterLikes", async (req, res, next) => {
  try {
    if (req.body.filter.anchorKey === "Most Liked") {
      const sortedPosts = await Posts.getMostLiked();
      res.status(200).json({ sortedPosts });
    }
    if (req.body.filter.anchorKey === "Least Liked") {
      const sortedPosts = await Posts.getLeastLiked();
      res.status(200).json({ sortedPosts });
    }
  } catch (err) {
    next(err);
